...
-  **Select default performance pad mode after Mixxx startup** Sets the performance pad mode that is active when Mixxx starts.

### Hot Cues Options

-  **Use transport pads as hotcues in Hot Cue mode** When enabled, the bottom row of transport pads (CUE, START, BACK, FWD) will function as hotcues 5-8 when the pad mode is set to 'Hot Cue'.
